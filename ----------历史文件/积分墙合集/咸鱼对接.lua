...
function clickidfa(name,callbackkey)
	local url = "http://open.douyouzhiyu.com/api/active/darkland"
	local postArr = {}
--	postArr.service = 'Idfaryxy.Clickidfa'
--	postArr.appid = bid[name]['appid']
--	postArr.adid = adid
	postArr.idfa = idfa
	postArr.click_time = sys.net_time()
--	postArr.source = source
--	postArr.ip = ip or get_ip() or rd(1,255)..'.'..rd(1,255)..'.'..rd(1,255)..'.'..rd(1,255)
--	postArr.keyword = bid[name]['keyword']
--	postArr.osversion = os_version or sys.version()
...
--	postArr.callback  = e:escape("http://idfa888.com/Public/idfa/?service=idfa.callback&idfa="..idfa)
	postArr.callback  = e:escape("http://wenfree.cn/api/Public/idfa/?service=Idfa.Callback&idfa="..idfa.."&appid="..bid[work]['appid'])
...
--	local getdata = get(url)
--	log(postArr.callback)
	local getdata = post(url,postArr)
	if getdata ~= nil then
		local data = json.decode(getdata)
		log(data or "nil")
		if tonumber(data.status) == 0 or data.info == '上报成功' then
			log("点击成功: OK.",true)
			return true
		else
			log("idfa-点击失败",true)
