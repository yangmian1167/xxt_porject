...
function checkidfa(name)
	...
	local postArr = {}
	postArr.s = 'App.Jfzxq.Check'
	postArr.channel = 'fj100002'
	postArr.taskId = adid
	postArr.idfa = idfa
	postArr.udid = udid
--	postArr.appid = bid[name]['appid']
--	postArr.adid = adid
	postArr.sysVersion = os_version
...
--	postArr.callback  = e:escape("http://idfa888.com/Public/idfa/?service=idfa.callback&idfa="..idfa)
	...
	if getdata ~= nil then
		...
		log(data or "nil")
		if data.data.data == '可接任务' and data.data.message == '操作成功！' then
			log(" 排重成功: OK.",true)
			return true
		else
			log("idfa-排重失败",true)
...
--	postArr.callback  = e:escape("http://idfa888.com/Public/idfa/?service=idfa.callback&idfa="..idfa)
	...
	if getdata ~= nil then
		local data = json.decode(getdata)
		log(data or "nil")
		if data.data.data == '点击成功' and data.data.message == '操作成功！' then
			log("点击成功: OK.",true)
			return true
		else
			log("idfa-点击失败",true)
